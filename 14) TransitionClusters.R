...
# ============================================================
# VARIABLE POUR COMPARAISONS EN 5 GROUPES
# Stable C1 / Late Migrant / Early Migrant / Transient / Stable C2
# ============================================================
...
df_all <- df_all %>%
  mutate(
    Group5 = case_when(
      Trajectory == "Stable C1"     ~ "Stable C1",
      Trajectory == "Late Migrant"  ~ "Late Migrant",
      Trajectory == "Early Migrant" ~ "Early Migrant",
      Trajectory == "Transient"     ~ "Transient",
      Trajectory == "Stable C2"     ~ "Stable C2",
      TRUE ~ NA_character_
    ),
    Group5 = factor(
      Group5,
      levels = c("Stable C1", "Late Migrant", "Early Migrant", "Transient", "Stable C2")
    )
  )
...
df_groups_comp <- df_all %>%
  filter(Group5 %in% c("Stable C1", "Late Migrant", "Early Migrant", "Transient", "Stable C2")) %>%
  droplevels()

df_groups_comp$Group5 <- factor(
  df_groups_comp$Group5,
  levels = c("Stable C1", "Late Migrant", "Early Migrant", "Transient", "Stable C2")
)
...
kw_age_groups <- kruskal_test(df_groups_comp, AgeMonths ~ Group5)
print(kw_age_groups)

# Taille d'effet globale (epsilon squared)
kw_age_groups_effsize <- kruskal_effsize(df_groups_comp, AgeMonths ~ Group5)
print(kw_age_groups_effsize)

# Post-hoc Dunn
dunn_age_groups <- df_groups_comp %>%
  dunn_test(AgeMonths ~ Group5, p.adjust.method = "holm")

print(dunn_age_groups)

# ============================================================
# ANNOTATIONS STATISTIQUES POUR AXE LOG
# ============================================================

group_levels <- levels(df_groups_comp$Group5)
...
stat_groups <- dunn_age_groups %>%
  ...
  arrange(xmin, xmax) %>%
  mutate(
    y.position = y_max_groups * seq(1.05, 1.25, length.out = n())
  )

y_top <- max(stat_groups$y.position, na.rm = TRUE) * 1.08
...
# Fisher exact : Sexe
tab_sex_groups <- table(df_groups_comp$Group5, df_groups_comp$Sex)
...
# ============================================================
# Boxplot âge avec axe Y log + barres manuelles
# ============================================================

# Hauteur des brackets
y_max_groups <- max(df_groups_comp$AgeMonths, na.rm = TRUE)

stat_groups_plot <- dunn_age_groups %>%
  filter(p.adj < 0.05) %>%
  mutate(
    group1 = as.character(group1),
    group2 = as.character(group2),
    x1 = match(group1, levels(df_groups_comp$Group5)),
    x2 = match(group2, levels(df_groups_comp$Group5)),
    label = case_when(
      p.adj < 0.001 ~ "***",
      p.adj < 0.01  ~ "**",
      p.adj < 0.05  ~ "*",
  ...
  arrange(x1, x2) %>%
  mutate(
    y = y_max_groups * c(1.12, 1.30, 1.50),
    y_tip = y / 1.04
  )

y_top <- max(stat_groups_plot$y, na.rm = TRUE) * 1.12

p_age_groups <- ggplot(df_groups_comp, aes(x = Group5, y = AgeMonths, fill = Group5)) +
  ...
  scale_y_log10(
    breaks = c(24, 60, 120, 180, 240, 300, 360, 420),
    labels = c("2", "5", "10", "15", "20", "25", "30", "35")
  ) +
  labs(
    x = NULL,
    y = "Age (years, log scale)"
  ) +
  
  # Barres horizontales
  geom_segment(
    data = stat_groups_plot,
  ...
  # Petits traits verticaux gauche
  geom_segment(
    data = stat_groups_plot,
...
p_sex_groups <- ggplot(sex_summary_groups, aes(x = Group5, y = prop, fill = Sex)) +
  ...
  labs(
    title = "Sex distribution by trajectory group",
    x = NULL,
    y = "Percentage (%)",
    fill = "Sex"
  ) +
...
# --- 4. Couleurs et noms des gait profiles ---
cols_trajectory <- c(
  "Stable C1"     = "#2C7FB8",  # bleu
  "Late Migrant"  = "darkred",  # rouge
  "Early Migrant" = "#FDB863",  # orange
  "Transient"     = "#9E9E9E",  # gris
  "Stable C2"     = "#31A354"   # vert
)

trajectory_legend_labels <- c(
...
# --- 6. Panel principal avec noms des domaines à gauche ---
...
# Panneaux texte pour les domaines
...
# Fonction pour créer un plot individuel pour chaque variable
make_var_plot <- function(var_name) {
  
  ggplot(
    df_panel_15 %>% filter(Variable == var_name),
    ...
    scale_fill_manual(
      values = cols_trajectory,
      labels = trajectory_legend_labels,
      drop = FALSE
    ) +
    labs(
      title = variable_labels[[var_name]],
      x = NULL,
      y = NULL,
      fill = "Trajectory"
    ) +
...
# Créer les 15 plots dans le bon ordre
plots_15 <- lapply(variables_15, make_var_plot)

# Assemblage : 5 lignes, chacune avec 1 domaine + 3 variables
panel_list <- list(
  domain_plots[[1]], plots_15[[1]],  plots_15[[2]],  plots_15[[3]],
  domain_plots[[2]], plots_15[[4]],  plots_15[[5]],  plots_15[[6]],
  domain_plots[[3]], plots_15[[7]],  plots_15[[8]],  plots_15[[9]],
  domain_plots[[4]], plots_15[[10]], plots_15[[11]], plots_15[[12]],
  domain_plots[[5]], plots_15[[13]], plots_15[[14]], plots_15[[15]]
)

p_panel_15 <- wrap_plots(panel_list, ncol = 4, byrow = TRUE) +
  plot_layout(
    widths = c(0.18, 1, 1, 1),
    heights = rep(1, 5),
    guides = "collect"
